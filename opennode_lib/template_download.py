...
class TemplateDownload(object):
    ...
    def __progressBarHook(self, downloaded, chunk, total):
        ...
        if (self.progressBarCounter % 10 == 0):
            ...
            bar = opennode_lib.progress.printProgressBar(percentage, size = 50, remainingTime = remaining)
            print bar,
            sys.stdout.flush()
            print "\r",
    ...
    def __resetProgressBarHook(self):
        ...
        delta = progressTime - self.progressStartTime
        remaining = int(delta)
        bar = opennode_lib.progress.printProgressBar(100, size = 50, remainingTime = remaining)
        print bar,
        sys.stdout.flush()
        print "\r"
