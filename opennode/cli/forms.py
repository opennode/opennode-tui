...
class EditVIF(Form):
    def __init__(self, screen, title, settings):
        ...
        self.settings = settings
        ar_debug(settings)
        if self.settings['edit_veth']:
            if type(self.settings['edit_veth']) == 'int':
                self.interface = self.settings['netif'][settings['edit_veth']]
            else:
                self.interface = self.settings['netif'][0]
        else:
        ...
        self.fields['gw'] = IpField('gw',
                                    ...
                                    width = 16)
        if self.settings['edit_veth']:
            self.fields['veth_name'] = VETHNameField('veth_name',
                                                     'eth0',
                                                     required  = True,
                                                     width = 8)
