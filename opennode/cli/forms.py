...
class NetworkSettings(Form):
        ...
        if self.interfaces:
            for iface in self.interfaces:
                ...
                if iface.get('gw'):
                    ...
                    gw = ''
                net_ifaces.append(' '.join([vlan,
                                            '%-8s' % iface['ifname'] if iface.get('type', None) == 'bridge'
                                            else iface['name'],
    ...
    def display(self):
        ...
        label3 = Label('* Default Route (Default RT)')
        label3.setColors(colorsets['BORDER'])
...
class AddVIF(Form):
    ...
    def display(self):
        ...
        rv = self.gf.runOnce()
        if bb.buttonPressed(rv) == 'back_net':
            return 'network'
...
class EditVIF(Form):
    def __init__(self, screen, title, settings):
        ...
        self.settings = settings
        self.screen = screen
        if 'editvif' in self.settings and not self.settings.get('add_veth', False):
            self.interface = self.settings['interfaces'][self.settings['editvif']]
        else:
            self.interface = {'type': 'bridge'}
        ...
        self.fields['gw'] = IpField('gw',
                                    ...
                                    required = False,
                                    width = 16)
        if self.interface.get('type', None) == 'bridge':
            if self.settings.get('add_veth', False):
                veth_name = self._get_available_ifname(self.settings)
            else:
                veth_name = self.interface.get('ifname')
            self.fields['veth_name'] = VETHNameField('ifname',
                                                     veth_name,
                                                     required = True,
                                                     width = 8)
        self.fields['vlan'] = IntegerField('vlan',
                                           self.interface.get('vlan', '1'),
                                           1, 4096,
                                           required=False, width=5)
        self.fields['burst'] = IntegerField('ingress_policing_burst',
                                            self.interface.get('ingress_policing_burst', ''),
                                            1, 99999, required=False, width=6)
        self.fields['rate'] = IntegerField('ingress_policing_rate',
                                           self.interface.get('ingress_policing_rate', ''),
                                           1, 99999, required=False, width=6)
        self.labels['name'] = self.interface.get('name', '')
        self.labels['host_bridge'] = self.interface.get('host_ifname', '')
        Form.__init__(self, screen, title, self.fields)

    def _get_available_ifname(self, settings):
        # Return eth0 if no veth interfaces exist,
        # else return first ifname with incremented number.
        interfaces = settings.get('interfaces', None)
        if not interfaces:
            return 'eth0'
        names = []
        for iface in interfaces:
            if iface['type'] == 'bridge':
                names.append(iface['ifname'])
        # Increment last/latest interface name
        if len(names) > 0:
            rv = names[-1][:-1] + str(int(names[-1][-1]) + 1)
            return rv
        else:
            return 'eth0'
    ...
    def display(self):
        ...
        if self.interface.get('type', None) == 'bridge':
            vethbox = HBox(2)
            vethbox.append(Label('Device name:     '), anchorLeft=1)
            vethbox.append(self.fields['veth_name'], anchorRight=1)
            hbl.append(vethbox)
        else:
        ...
        vb.append(self.fields['vlan'], padding=(0, 0, 2, 0))
        vb.append(Label('BW Limit (Mbps):'), padding=(0, 0, 1, 0))
        vb.append(self.fields['rate'], padding=(0, 0, 2, 0))
        vb.append(Label('BW Burst (Mbps):'), padding=(0, 0, 1, 0))
        vb.append(self.fields['burst'])
        self.gf.add(vb, 0, 3)
        bb = ButtonBar(self.screen, (('Cancel', 'back_net', 'F12'), ('Ok', 'network_vif')))
    ...
    def validate(self):
        ...
        if self.errors:
            return not self.errors
        if self.data.get('dhcp', None):
            if self.interface.get('type', None) == 'bridge':
                iface = {'ifname': self.data['ifname'],
                         'managed': yn_encoding[self.data['managed']],
                         'dhcp': 'yes',
                         'vlan': self.data['vlan'],
                         'ingress_policing_burst': self.data['ingress_policing_burst'],
                         'ingress_policing_rate': self.data['ingress_policing_rate']}
                self.data = iface
        else:
            if 'vif_mac' in self.data:
                del (self.data['vif_mac'])
        if self.data.get('mac', None):
            if 'autogenerated' in self.data['mac']:
                self.data.pop('mac', None)
        # We have new veth device, add flag for logic
        if self.settings.get('add_veth', False):
            self.data['new_veth'] = True
            self.data['type'] = 'bridge'
        return not self.errors
...
class VETHNameField(Field):
    """Name field for VETH devices.
    Validation requires last character to be a number"""
    def __init__(self, name, default, required=True, width=20, display_name=None):
        Field.__init__(self, name, default, width, required=required,
    ...
    def validate(self):
        if Field.validate(self):
            if self.value()[-1] not in string.digits:
                return False
        return True
